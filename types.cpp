...
PublicTransport::PublicTransport(std::string name, std::string model, std::string vehicle_number, std::string organization)
{
...
PublicTransport::PublicTransport(std::istream &in, std::ostream &out) : IVehicle(in, out)
{

    out << "Owner organization: ";
    in >> organization;
}
void PublicTransport::stream(std::ostream &str)
{
    IVehicle::stream(str);
    str << "Owner organization: "
        << organization << '\n';
}
void PublicTransport::stream_table(std::ostream &str)
{
    IVehicle::stream_table(str);
    str << std::setw(18) << " | " << std::setw(17) << " | " << std::setw(15) << truncate(organization) << " |" << '\n';
}
template <class Archive>
void PublicTransport::serialize(Archive &ar)
{
    ar(cereal::base_class<IVehicle>(this), organization);
}
CEREAL_REGISTER_TYPE(PersonalTransport);
CEREAL_REGISTER_TYPE(PublicTransport);
CEREAL_REGISTER_POLYMORPHIC_RELATION(IVehicle, PersonalTransport);
CEREAL_REGISTER_POLYMORPHIC_RELATION(IVehicle, PublicTransport);